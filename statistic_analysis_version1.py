...
class State_analysis(object):
    ...
    def __init__(self, state):
        ...
        self.ndvi_inraster_path = os.path.normcase("D:/Environment Factors/NDVI 3000")
        if os.path.exists(os.path.join(self.state_path, "parks_have_been_created.txt")):
            self.parks_created = True
        else:
            self.parks_created = False
        if os.path.exists(os.path.normcase(os.path.join(self.state_path, "ndvi results/ndvi parks.csv"))):
            self.ndvi_processed = True
        else:
            self.ndvi_processed = False
    ...
    def get_statistic_task11(self):
        ...
        for input_directory in input_file_dir_list: # iteret every park
            ...
            for input_file in input_file_list:
                temp_txt_file = os.path.join(output_path, "aidsbycacnty.txt")
                arcpy.ExportXYv_stats(input_file, "elevation", "SPACE", temp_txt_file, "ADD_FIELD_NAMES")
                with open(temp_txt_file, "rb") as txt_rd:
                    txt_rd.readline()
                    value = txt_rd.readline().strip().split(" ")
                    mean = value[2]
                    temp_mean_list.append(mean)
            data_dict[input_directory_name] = temp_mean_list
        output_name = os.path.join(output_path, "near_elevation_ave_aprsep.csv")
        with file(output_name, "wb") as output_fd:
            output_writer = csv.writer(output_fd)
            output_writer.writerow([' '] + year_labels)
            for key in sorted(data_dict.keys()):
        ...
        for input_directory in input_file_dir_list: # iteret every park
            ...
            for input_file in input_file_list:
                ...
                with open(temp_txt_file, "rb") as txt_rd:
                    ...
                    temp_mean_list.append(value)
            data_dict[input_directory_name] = temp_mean_list
        output_name = os.path.join(output_path, "near_elevation_aot_aprsep.csv")
        with file(output_name, "wb") as output_fd:
            output_writer = csv.writer(output_fd)
            output_writer.writerow([' '] + year_labels)
            for key in sorted(data_dict.keys()):
        ...
        for input_directory in input_file_dir_list: # iteret every park
            ...
            for input_file in input_file_list:
                ...
                with open(temp_txt_file, "rb") as txt_rd:
                    ...
                    temp_mean_list.append(value)
            data_dict[input_directory_name] = temp_mean_list
        output_name = os.path.join(output_path, "near_elevation_prcp_aprsep.csv")
        with file(output_name, "wb") as output_fd:
            output_writer = csv.writer(output_fd)
            output_writer.writerow([' '] + year_labels)
            for key in sorted(data_dict.keys()):
        ...
        for input_directory in input_file_dir_list: # iteret every park
            ...
            for input_file in input_file_list:
                ...
                with open(temp_txt_file, "rb") as txt_rd:
                    ...
                    temp_mean_list.append(value)
            data_dict[input_directory_name] = temp_mean_list
        output_name = os.path.join(output_path, "near_elevation_temp_aprsep.csv")
        with file(output_name, "wb") as output_fd:
            output_writer = csv.writer(output_fd)
            output_writer.writerow([' '] + year_labels)
            for key in sorted(data_dict.keys()):
...
#    create_state_parks()
    ...
    for state_dir in state_dir_list:
        ...
        if not state_instance.parks_created:
            state_instance.create_parks()
        if not state_instance.ndvi_processed:
...
#            function()
 ...
 #       for index in range(1,5):
  #          task_name = "state_instance.get_statistic_task1" + str(index)
#            function = eval(task_name)
 #           function()
